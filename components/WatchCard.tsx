...
export function WatchCard({ watch }: { watch: Watch }) {
  ...
  const tagline = t(`watches.${watch.id}.tagline`);
  const description = t(`watches.${watch.id}.description`);
  const specs = t(`watches.${watch.id}.specs`);
  const badge = t('collection.limitedBadge');

  const msg = `Hi, I'm interested in ${brand} · ${model} (${badge}).`;
  ...
  return (
    <article className="group flex flex-col border border-white/5 bg-ink-soft transition hover:border-gold/40">
      <div className="relative aspect-[4/5] overflow-hidden bg-ink-muted">
        <img
          src={watch.image}
          alt={`${brand} · ${model} — Palm dial automatic watch`}
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-2 text-sm italic text-white/60">{tagline}</p>

        <p className="mt-6 flex-1 text-sm leading-relaxed text-white/60">
          {description}
        </p>

        <p className="mt-6 border-t border-white/5 pt-6 text-[11px] leading-relaxed tracking-wide text-white/50">
          {specs}
        </p>
